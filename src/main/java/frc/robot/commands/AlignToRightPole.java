...
public class AlignToRightPole extends Command {
    ...
    public AlignToRightPole(SwerveSubsystem drivebase, LimeLights limeLights) {
    ...
    public void execute() {
        ...
        if (!limeLights.isAprilTagDetected(true)) {
            drivebase.stop();
            return;
        }

        // ✅ Retrieve botPose data **every cycle**
        double[] botPose = limeLights.getAprilTagPose(true);
